...
namespace zyzn{
    namespace status{
        ...
        m_pubCtrl(nullptr),m_sts(-1){
            m_pubCtrl  = info::CParam::m_glbNode->create_publisher<std_msgs::msg::UInt8>
            ("inner/control/weapon",10);
            m_subSts = info::CParam::m_glbNode->create_subscription<std_msgs::msg::UInt8>(
        ...
        void Weapon::weaponCtrl(int enable,int cmd){
            ...
            weapon.data = enable;//WEAPON_FUN_NUM*enable+cmd;
            m_pubCtrl->publish(weapon);
            RCLCPP_INFO(rclcpp::get_logger("weaponCtrl"), "ctrl weapon enable:%d cmd:%d msgdata:%d",enable,cmd,weapon.data);
        }

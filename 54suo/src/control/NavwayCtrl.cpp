...
namespace zyzn{
    namespace ctrl{
        ...
        const BT::NodeConfig& conf):StatefulActionNode(instance_name,conf){
            if(!m_s_navCtrl){
                m_s_navCtrl = info::CParam::m_glbNode->create_client<custom_msgs::srv::CommandInt>(
        ...
        NodeStatus CNavwayCtrl::onStart(){
            if(m_s_navCtrl->wait_for_service(std::chrono::milliseconds(50))){
                RCLCPP_INFO(rclcpp::get_logger("navctrl"),"service is ready");
                return NodeStatus::RUNNING;
            }
            RCLCPP_WARN(rclcpp::get_logger("navctrl"),"service not ready");
        ...
        NodeStatus CNavwayCtrl::onRunning(){
            ...
            RCLCPP_INFO(rclcpp::get_logger("navctrl"),"navctrl frame:%d command:%d",request->frame,request->command);

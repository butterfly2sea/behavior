...
namespace zyzn{
    namespace ctrl{
        ...
        CJoystick::CJoystick(const std::string& name, const NodeConfig& config):SyncActionNode(name,config),
            ...
            m_yawCh(YawCh){
                m_s_startJoy = true;
                m_pubManCtrl = info::CParam::rosNode()->create_publisher<mavros_msgs::msg::ManualControl>("/mavros/manual_control/send",1);
                m_subJoy = info::CParam::rosNode()->create_subscription<sensor_msgs::msg::Joy>("/inner/control/joystick",rclcpp::SensorDataQoS(),std::bind(&CJoystick::joyCB,

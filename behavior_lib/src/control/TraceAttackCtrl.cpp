    ...
    namespace ctrl{
        ...
        const BT::NodeConfig& conf):StatefulActionNode(instance_name,conf){
            if(!m_s_traceAttackCtrl){
                m_s_traceAttackCtrl = info::CParam::rosNode()->create_client<custom_msgs::srv::CommandInt>(
        ...
        NodeStatus CTraceAttackCtrl::onRunning(){
            ...
            txtLog().info(THISMODULE "traceattack ctrl frame:%d command:%d current:%d",request->frame,request->command,
            request->current);

...
namespace zyzn{
    namespace manage{
        /**
         * @brief json任务解析 依据收到的json任务，从中解析出任务参数，并个触发对应的执行信号
         * @author zyzn
        */
        class CActionsMgr{
        public:      
            ...
            */
            virtual void processStage(const Json::Value &json_str);

            /**
             * @brief 收到的json任务回调函数,获取到任务信息后进行json任务处理
             * @param in msg json任务消息内容
            */
            virtual void jsonTaskCallback(const std_msgs::msg::String::SharedPtr msg);
            protected:

            virtual bool getFormoffset(int grpId,const Json::Value &actions,std::vector<geometry_msgs::msg::Point32> & rslt);

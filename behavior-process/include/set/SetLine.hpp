        ...
        class CSetLine : public SyncActionNode{
           ...
           template <typename IdsTyp>
            ...
            for(const uint8_t & id:exIds){
                auto it = std::find(info::CParam::m_sCurStage.head.ids.begin(),
                info::CParam::m_sCurStage.head.ids.end(),id);
                if(info::CParam::m_sCurStage.head.ids.end() != it){
                    idxs.insert(it-info::CParam::m_sCurStage.head.ids.begin());
                }      

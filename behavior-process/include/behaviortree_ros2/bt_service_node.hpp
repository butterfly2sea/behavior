  ...
  bool RosServiceNode<T>::createClient(const std::string& service_name)
{
  ...
  if(!service_client_){
      ...
      RCLCPP_INFO(node_->get_logger(), "%s: no",
                 service_name.c_str());
      prev_service_name_ = service_name;
  }else{
    RCLCPP_INFO(node_->get_logger(), "%s: yes",
                 service_name.c_str());
  }
  ...
  m_foundSrv = service_client_->wait_for_service(service_timeout_);
  if(!m_foundSrv)
  {
    RCLCPP_WARN(node_->get_logger(), "%s: Service with name '%s' is not reachable.",
  ...
  NodeStatus RosServiceNode<T>::tick()
{
  ...
  if (status() == BT::NodeStatus::IDLE)
  {
    ...
    response_ = {};
    //RCLCPP_INFO(node_->get_logger(), "send request before");
    
    if( !setRequest(m_request) )
    {
      RCLCPP_ERROR(node_->get_logger(), "send request failed! name");
      return CheckStatus( onFailure(INVALID_REQUEST) );
    }

    future_response_ = service_client_->async_send_request(m_request);
    time_request_sent_ = node_->now().seconds();
    RCLCPP_INFO(node_->get_logger(), "send request after %f",time_request_sent_);
  ...
  if (status() == NodeStatus::RUNNING)
  {
    ...
    if( !response_received_ )
    {
      ...
      double timeout =  double(service_timeout_.count()) / 1000;
      ...
      {
        if( (node_->now().seconds() - time_request_sent_) > timeout )
        {
          RCLCPP_ERROR(node_->get_logger(), "srv spin until future failed srv: %s send:%f now:%f len:%f",prev_service_name_.c_str(),
          time_request_sent_, node_->now().seconds(), node_->now().seconds() - time_request_sent_);
    ...
    if(response_received_){
      NodeStatus sts =  CheckStatus( onResponseReceived( response_ ) );
      if(m_repeatReq){
        //resetStatus();

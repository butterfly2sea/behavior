...
namespace zyzn{
    namespace algorithm{
        ...
        void CUtility::getVerPtFrmLine(const Point & pt1,const Point & pt2,Point &out1,Point &out2,
        float dis){
            if(abs(pt2.x()-pt1.x())>1e-6){
                
                float angle = atan((pt1.x()-pt2.x())/(pt2.y()-pt1.y()));
                
                out1.x(pt1.x() + dis*cos(angle));
                out1.y(pt1.y() + dis*sin(angle));
                out2.x(pt1.x() - dis*cos(angle));
                out2.y(pt1.y() - dis*sin(angle));
                return;
            }
            out1.x(pt1.x() - dis);
            out1.y(pt1.y());
       ...
       CUtility::Point CUtility::getNearPt(const float & x1,const float & y1,const float & x2,const float & y2,
        const float & xsrc,const float & ysrc,float  dis){
            Point pt1(y1,x1);
            Point pt2(y2,x2);
            Point ptSrc(ysrc,xsrc);
            Point out1,out2;
            getVerPtFrmLine(pt1,pt2,out1,out2,dis);
            return getNearPt(out1,out2,ptSrc);
        }

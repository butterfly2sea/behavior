...
namespace zyzn{
    namespace ctrl{
        ...
        StatefulActionNode(name,conf),m_tickCount(0){
            if(!m_s_cltLockCtrl){
                m_s_cltLockCtrl = info::CParam::m_glbNode->create_client<custom_msgs::srv::CommandBool>(
        ...
        NodeStatus CLockCtrl::onStart(){
            if(m_s_cltLockCtrl->wait_for_service(std::chrono::milliseconds(50))){
                RCLCPP_INFO(rclcpp::get_logger("LockCtrl"),"service is ready");
        ...
        NodeStatus CLockCtrl::onRunning(){
            ...
            if(get::CGetlocation::simpVehi().lock == m_expState){
                RCLCPP_INFO(rclcpp::get_logger("LockCtrl"),"ctrl:%d success",m_expState);

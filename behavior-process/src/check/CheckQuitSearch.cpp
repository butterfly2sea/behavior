        ...
        NodeStatus CCheckQuitSearch::tick(){
            if(m_lastMsg && m_lastMsg->objs.size()>0){
                ...
                if(abs(tn-m_lastMsg->header.stamp.sec )<2){
                    ...
                    if(0 != set::CSetTraceAttackObj::tgtInfo().tgtId){
                        for(auto obj:m_lastMsg->objs){
                            if(obj.id == set::CSetTraceAttackObj::tgtInfo().tgtId){
                                set::CSetTraceAttackObj::objLoc() = obj;
                                fd = true;
                                break;
                            }
                        }
                    }else{//如没指定目标id，则认为只要收到目标位置就找到了目标
                        set::CSetTraceAttackObj::objLoc() = m_lastMsg->objs[0];
                        fd = true;
                    }
                    if(m_autoNxt && fd){

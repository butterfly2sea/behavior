...
namespace zyzn{
    namespace status{
        ...
        void CCommandStatus::sendCmdRsp(){
            ...
            m_s_msgCmdRsp.id = zyzn::info::CParam::m_sSelfId;
        ...
        void CCommandStatus::init(){
            if(!m_s_pubResp){
                m_s_pubResp = info::CParam::m_glbNode->create_publisher<custom_msgs::msg::CommandResponse>
            ("outer/command/response",10);
            }
            if(!m_s_pubStsTask){
                m_s_pubStsTask = info::CParam::m_glbNode->create_publisher<custom_msgs::msg::StatusTask>
                ("outer/information/status_task",10);
            }
        }

        void CCommandStatus::initResp(int subCmd,ECtrlType ctrl){
            m_s_msgCmdRsp.id = info::CParam::m_sSelfId;

...
BT::NodeStatus SetLineParameters::tick() {
  ...
  if (VEHICLE_TYP & set_type) {
    auto vehicle_type =
        context()->hasParameter("vehiType") && context()->getParameter("vehiType").get<std::string>() == "多旋翼"
        ? VehicleType::Coper : VehicleType::FixWing;
  ...
  // 发布速度
  if (SPD & set_type) {
    float speed = std::stof(context()->getParameter("spd").get<std::string>());
    context()->setSpeed(speed);
    std_msgs::msg::Float32 msg;
    msg.data = speed;
    ros()->publish<std_msgs::msg::Float32>(ros_interface::topics::SET_GROUP_SPEED, msg); // 发布速度
  }

  // 发布到点距离
  if (ARV_DIS & set_type) {
    float arv_dis = std::stof(context()->getParameter("arvDis").get<std::string>());
  ...
  if (FORM & set_type) {
    ros()->publish<custom_msgs::msg::ParamShort>(ros_interface::topics::SET_FORMATION,
                                                 context()->getTaskStage().form);
  }
  if (GROUP & set_type) {
